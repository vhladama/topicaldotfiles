...
Bootstrap dotfiles onto new system.
Symlink relevant topics from topics-available to topics-enabled to activate.
for example, on a system where one will be doing ruby development, ruby topic can install rvm, rubocop, etc.

Installation:

```
bash -c "$(curl -fsSL https://raw.githubusercontent.com/vhladama/topicaldotfiles/baseTopics/bin/dotfiles)" && source ~/.bashrc
```
